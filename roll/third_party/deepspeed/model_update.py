...
def gather_deepspeed_weights(model, ds_config, buffer_size, is_lora=False):
    ...
    if is_lora:
        if not is_zero3:
            ...
            # For Zero3, get_peft_model_state_dict returns 1D partitioned tensor copies
            # without ds_id, so GatheredParameters can't gather them. We need actual
            # parameter references from the model that GatheredParameters can work with.
            adapter_name = "default"
            named_params = []
            for name, param in model.named_parameters():
                if "lora_" in name and adapter_name in name:
                    clean_name = name.replace(f".{adapter_name}", "")
                    named_params.append((clean_name, param))
    else:
    ...
    def _colocated_model_update(self):
        ...
        for named_weights in gather_deepspeed_weights(
            ...
            if co_infer_rank == 0 and self._co_infer_worker is not None:
                refs.append(self._co_infer_worker.update_parameter_in_bucket.remote(
